...
REDIS_URLS: list[str] = ["redis://154.54.100.113:20004", "redis://64.247.206.65:40366", "redis://64.247.206.73:40067"]
    ...
    async def ensure_checkpoint_loaded(
        ...
        try:
            ...
            while waited < max_wait_seconds:
                # Check if partial download directory exists (download in progress)
                ...
                # Check if final checkpoint directory exists
                if checkpoint_dir.exists() and checkpoint_dir.is_dir():
                    # Check for essential files based on actual checkpoint structure
                    # Checkpoints have: config.json, metadata.json, and model file(s)
                    config_file = checkpoint_dir / "config.json"
                    metadata_file = checkpoint_dir / "metadata.json"
                    
                    # Model can be either:
                    # 1. Single file: model.safetensors
                    # 2. Sharded: model.safetensors.index.json + model-*.safetensors files
                    single_model = (checkpoint_dir / "model.safetensors").exists()
                    index_file = checkpoint_dir / "model.safetensors.index.json"
                    model_shards = list(checkpoint_dir.glob("model-*.safetensors"))
                    has_sharded_model = index_file.exists() and len(model_shards) > 0
                    
                    has_config = config_file.exists()
                    has_metadata = metadata_file.exists()
                    has_model = single_model or has_sharded_model
                    
                    if has_config and has_metadata and has_model:
                        ...
                        logger.info(
                            ...
                            f"(config={has_config}, metadata={has_metadata}, "
                            f"model={'single' if single_model else 'sharded'})"
                        )
                        break
...
async def generate_group_rollouts(
    ...
    try:
        ...
        # Log randomness_hex for debugging sketch mismatches
        logger.info(
            f"🎲 WORKER RANDOMNESS: window={window_start} group={group_id} "
            f"randomness_hex={randomness_hex[:32]}... (len={len(randomness_hex)}) use_drand={use_drand}"
        )

        # Environment factory matching default miner
        def _env_factory():
            return create_env()

        # Generate GRPO rollouts using AgentEnvLoop (same as default miner)
        gen_start = time.time()
        logger.info(f"Rollout per problem: {ROLLOUTS_PER_PROBLEM}")
